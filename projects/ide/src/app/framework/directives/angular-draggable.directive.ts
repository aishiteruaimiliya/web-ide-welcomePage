...
export class AngularDraggableDirective implements OnInit, OnChanges {
    ...
    @Input() region: 'west'|'east'|'north'|'south';
    ...
    boundsCheck() {
        if (this.bounds) {
            ...
            let r = boundary.right, l = boundary.left;
    ...
    private toggleMask(show: boolean) {
        if (show) {
            this.mask = this.renderer.createElement('div');
            this.renderer.addClass(this.mask, 'drag-mask');
            this.renderer.insertBefore(this.el.nativeElement.parentElement, this.mask, this.el.nativeElement);
        } else if (this.mask) {
            this.renderer.removeChild(this.mask.parentElement, this.mask);
        }
    }
    ...
    onMouseDown(event: MouseEvent | TouchEvent) {
        ...
        this.renderer.setStyle(target, 'background', '#495057');
    }
